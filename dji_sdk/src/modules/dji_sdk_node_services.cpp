...
bool
DJISDKNode::queryVersionCallback(dji_sdk::QueryDroneVersion::Request&  request,
...
bool
DJISDKNode::stereo240pSubscriptionCallback(
  dji_sdk::Stereo240pSubscription::Request&  request,
  dji_sdk::Stereo240pSubscription::Response& response)
{
  ...
  vehicle->advancedSensing->subscribeStereoImages(
    &image_select, &publish240pStereoImage, this);
  ...
  {
    response.result = false;
    ROS_WARN("Stereo 240p subscription service failed, please check your "
             "request content.");
  }
...
bool
DJISDKNode::stereoDepthSubscriptionCallback(
  dji_sdk::StereoDepthSubscription::Request&  request,
  dji_sdk::StereoDepthSubscription::Response& response)
{
  ...
  {
    this->stereo_subscription_success = false;
    vehicle->advancedSensing->subscribeFrontStereoDisparity(
      &publish240pStereoImage, this);
  }
  else
  {
...
bool
DJISDKNode::stereoVGASubscriptionCallback(
  dji_sdk::StereoVGASubscription::Request&  request,
  dji_sdk::StereoVGASubscription::Response& response)
{
  ...
  if (request.front_vga == 1)
  {
    ...
    vehicle->advancedSensing->subscribeFrontStereoVGA(
      request.vga_freq, &publishVGAStereoImage, this);
    ros::Duration(1).sleep();
  }
  ...
  {
    response.result = false;
    ROS_WARN("Stereo VGA subscription service failed, please check your "
             "request content.");
  }
...
bool
DJISDKNode::setupCameraStreamCallback(
  dji_sdk::SetupCameraStream::Request&  request,
  dji_sdk::SetupCameraStream::Response& response)
{
  ...
  if (request.cameraType == request.FPV_CAM)
  {
    if (request.start == 1)
    {
      result = vehicle->advancedSensing->startFPVCameraStream(
        &publishFPVCameraImage, this);
    }
    else
    {
  ...
  else if (request.cameraType == request.MAIN_CAM)
  {
    if (request.start == 1)
    {
      result = vehicle->advancedSensing->startMainCameraStream(
        &publishMainCameraImage, this);
    }
    else
    {
