...
DJISDKNode::initPublisher(ros::NodeHandle& nh)
{
  ...
  control_device_publisher =
    nh.advertise<dji_sdk::ControlDevice>("dji_sdk/control_device", 10);
  ...
  rtk_yaw_publisher = nh.advertise<std_msgs::Int16>("dji_sdk/rtk_yaw", 10);
  ...
  stereo_240p_front_left_publisher = nh.advertise<sensor_msgs::Image>(
    "dji_sdk/stereo_240p_front_left_images", 10);

  stereo_240p_front_right_publisher = nh.advertise<sensor_msgs::Image>(
    "dji_sdk/stereo_240p_front_right_images", 10);

  stereo_240p_down_front_publisher = nh.advertise<sensor_msgs::Image>(
    "dji_sdk/stereo_240p_down_front_images", 10);

  stereo_240p_down_back_publisher = nh.advertise<sensor_msgs::Image>(
    "dji_sdk/stereo_240p_down_back_images", 10);

  stereo_240p_front_depth_publisher = nh.advertise<sensor_msgs::Image>(
    "dji_sdk/stereo_240p_front_depth_images", 10);

  stereo_vga_front_left_publisher = nh.advertise<sensor_msgs::Image>(
    "dji_sdk/stereo_vga_front_left_images", 10);

  stereo_vga_front_right_publisher = nh.advertise<sensor_msgs::Image>(
    "dji_sdk/stereo_vga_front_right_images", 10);
  ...
  else if (telemetry_from_fc == USE_SUBSCRIBE)
  {
    ...
    {
      ROS_INFO("align_time_with_FC set to false. We will use ros time to time "
               "stamp messages!");
    }
    else
    {
      ROS_INFO("align_time_with_FC set to true. We will time stamp messages "
               "based on flight controller time!");
    }
    ...
    angularRate_publisher = nh.advertise<geometry_msgs::Vector3Stamped>(
      "dji_sdk/angular_velocity_fused", 10);

    acceleration_publisher = nh.advertise<geometry_msgs::Vector3Stamped>(
      "dji_sdk/acceleration_ground_fused", 10);

    trigger_publisher =
      nh.advertise<sensor_msgs::TimeReference>("dji_sdk/trigger_time", 10);
...
DJISDKNode::dataBroadcastCallback()
{
  ...
  uint16_t flag_has_rc =
    isM100() ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_RC)
  ...
  if (flag_has_rc)
  {
    ...
    rc_joy.axes.push_back(
      static_cast<float>(vehicle->broadcast->getRC().roll / 10000.0));
    rc_joy.axes.push_back(
      static_cast<float>(vehicle->broadcast->getRC().pitch / 10000.0));
    rc_joy.axes.push_back(
      static_cast<float>(vehicle->broadcast->getRC().yaw / 10000.0));
    rc_joy.axes.push_back(
      static_cast<float>(vehicle->broadcast->getRC().throttle / 10000.0));
  ...
  {
    R_FRD2NED.setRotation(
      tf::Quaternion(vehicle->broadcast->getQuaternion().q1,
      ...
      (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_A))
  {
    ...
    imu.linear_acceleration.x =
      vehicle->broadcast->getAcceleration().x * gravity_const;
    imu.linear_acceleration.y =
      -vehicle->broadcast->getAcceleration().y * gravity_const;
    imu.linear_acceleration.z =
      -vehicle->broadcast->getAcceleration().z * gravity_const;
  ...
  if (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_POS)
  {
    ...
    if (local_pos_ref_set)
    {
      ...
      gpsConvertENU(local_pos.point.x, local_pos.point.y, gps_pos.longitude,
                    gps_pos.latitude, this->local_pos_ref_longitude,
                    this->local_pos_ref_latitude);
  ...
  uint16_t flag_has_battery =
    isM100()
      ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_BATTERY)
  ...
  if (flag_has_battery)
  {
    ...
    msg_battery_state.voltage =
      vehicle->broadcast->getBatteryInfo().voltage / 1000.0;
    msg_battery_state.current = NAN;
    msg_battery_state.percentage =
      vehicle->broadcast->getBatteryInfo().percentage;
    msg_battery_state.charge = NAN;

    msg_battery_state.design_capacity = NAN;
    msg_battery_state.power_supply_health =
      msg_battery_state.POWER_SUPPLY_HEALTH_UNKNOWN;
    msg_battery_state.power_supply_status =
      msg_battery_state.POWER_SUPPLY_STATUS_UNKNOWN;
    msg_battery_state.power_supply_technology =
      msg_battery_state.POWER_SUPPLY_TECHNOLOGY_UNKNOWN;
    msg_battery_state.present =
      (vehicle->broadcast->getBatteryInfo().voltage != 0);
    battery_state_publisher.publish(msg_battery_state);
  }

  uint16_t flag_has_status =
    isM100()
      ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_STATUS)
  ...
  uint16_t flag_has_gimbal =
    isM100()
      ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_GIMBAL)
                            ...
                            DJI::OSDK::UserData userData)
{
  ...
  Telemetry::TimeStamp packageTimeStamp =
    *(reinterpret_cast<Telemetry::TimeStamp*>(data));
  ...
  dji_sdk::ControlDevice msg_control_device;
  msg_control_device.header.stamp = ros::Time::now();
  //  msg_control_device.header.frame_id = ""
  //  msg_control_device.header.seq
  auto ctrldev =
    vehicle->subscribe->getValue<Telemetry::TOPIC_CONTROL_DEVICE>();
  msg_control_device.controlMode   = ctrldev.controlMode;
  msg_control_device.controlDevice = ctrldev.deviceStatus;
  msg_control_device.flightStatus  = ctrldev.flightStatus;
  msg_control_device.vrcStatus     = ctrldev.vrcStatus;
  p->control_device_publisher.publish(msg_control_device);

  if (p->rtkSupport)
  {
    Telemetry::TypeMap<Telemetry::TOPIC_RTK_POSITION>::type
      rtk_telemetry_position =
        vehicle->subscribe->getValue<Telemetry::TOPIC_RTK_POSITION>();
    Telemetry::TypeMap<Telemetry::TOPIC_RTK_VELOCITY>::type
      rtk_telemetry_velocity =
    ...
    Telemetry::TypeMap<Telemetry::TOPIC_RTK_YAW_INFO>::type
      rtk_telemetry_yaw_info =
        vehicle->subscribe->getValue<Telemetry::TOPIC_RTK_YAW_INFO>();
    Telemetry::TypeMap<Telemetry::TOPIC_RTK_POSITION_INFO>::type
      rtk_telemetry_position_info =
                            ...
                            DJI::OSDK::UserData userData)
{
  ...
  if (p->local_pos_ref_set)
  {
    ...
    p->gpsConvertENU(local_pos.point.x, local_pos.point.y, gps_pos.longitude,
                     gps_pos.latitude, p->local_pos_ref_longitude,
                     p->local_pos_ref_latitude);
  ...
  *            -10000                         -10000
  *
  *   In this code, before publish, RC is transformed to M100 style to be
 ...
 *         controller to stamp all published ros messages. The reason is that
 * the
 *         flight controller is running a real time system, while the ros time
 * can
 *         be affected by OS scheduling depending on system load.
 */

void
DJISDKNode::alignRosTimeWithFlightController(ros::Time now_time, uint32_t tick)
{
  ...
  if (curr_align_state == ALIGNING)
  {
    ...
    double dt =
      std::fabs((now_time - (base_time + _TICK2ROSTIME(tick))).toSec());
    ...
    {
      base_time = now_time - _TICK2ROSTIME(tick);
      ROS_INFO("[dji_sdk] ***** Time difference out of bound after %d samples, "
               "retried %d times, dt=%.3f... *****",
...
#ifdef ADVANCED_SENSING
void
DJISDKNode::publish240pStereoImage(Vehicle* vehicle, RecvContainer recvFrame,
                                   DJI::OSDK::UserData userData)
{
  ...
  for (int pair_idx = 0; pair_idx < CAMERA_PAIR_NUM; ++pair_idx)
  {
    for (int dir_idx = 0; dir_idx < IMAGE_TYPE_NUM; ++dir_idx)
    {
      ...
      uint8_t bit_val =
        (recvFrame.recvData.stereoImgData->img_desc >> bit_location) & 1;
      ...
      if (bit_val)
      {
        ...
        img.header.frame_id =
          recvFrame.recvData.stereoImgData->img_vec[img_idx].name;
        memcpy((char*)(&img.data[0]),
               recvFrame.recvData.stereoImgData->img_vec[img_idx++].image,
               240 * 320);
...
void
DJISDKNode::publishVGAStereoImage(Vehicle* vehicle, RecvContainer recvFrame,
                                  DJI::OSDK::UserData userData)
{
  DJISDKNode* node_ptr = (DJISDKNode*)userData;
  ...
  memcpy((char*)(&img.data[0]), recvFrame.recvData.stereoVGAImgData->img_vec[0],
         480 * 640);
  node_ptr->stereo_vga_front_left_publisher.publish(img);

  img.header.frame_id = "vga_right";
  memcpy((char*)(&img.data[0]), recvFrame.recvData.stereoVGAImgData->img_vec[1],
         480 * 640);
...
void
DJISDKNode::publishFPVCameraImage(CameraRGBImage rgbImg, void* userData)
{
  DJISDKNode* node_ptr = (DJISDKNode*)userData;
...
void
DJISDKNode::publishMainCameraImage(CameraRGBImage rgbImg, void* userData)
{
  DJISDKNode* node_ptr = (DJISDKNode*)userData;
