 ...
 */
void
DJISDKNode::flightControl(uint8_t flag, float xSP, float ySP, float zSP,
                          float yawSP)
{
  ...
  if (FRAME == Control::HORIZONTAL_GROUND)
  {
    ...
    {
      // ROS_DEBUG("GROUND frame is specified, but angle and rate command is
      // generated in body frame");
      xCmd = RAD2DEG(xSP);
      yCmd = RAD2DEG(-ySP);
    }
    ...
    {
      // ROS_WARN_THROTTLE(1.0, "GROUND frame is specified, but thrust command
      // is generated in body frame");
      zCmd = zSP;
    }
  }
  else if (FRAME == Control::HORIZONTAL_BODY)
  {
    ...
    if ((VERT == Control::VERTICAL_VELOCITY) ||
        (VERT == Control::VERTICAL_POSITION))
    {
      // ROS_WARN_THROTTLE(1.0, "BODY frame is specified, but hight and
      // z-velocity is generated in ground frame");
      zCmd = zSP;
    }
    else
    {
...
{
  uint8_t flag =
    (Control::VERTICAL_POSITION | Control::HORIZONTAL_POSITION |
     Control::YAW_ANGLE | Control::HORIZONTAL_GROUND | Control::STABLE_ENABLE);
...
{
  uint8_t flag =
    (Control::VERTICAL_VELOCITY | Control::HORIZONTAL_VELOCITY |
...
{
  uint8_t flag =
    (Control::VERTICAL_POSITION | Control::HORIZONTAL_ANGLE |
