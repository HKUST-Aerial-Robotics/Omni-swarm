  ...
  dji_sdk::MissionWpUpload::Response& response)
{
  ...
  wpInitData.indexNumber =
    (unsigned char)request.waypoint_task.mission_waypoint.size();
  ...
  wpInitData.finishAction =
    (unsigned char)request.waypoint_task.action_on_finish;
  wpInitData.executiveTimes =
    (unsigned char)request.waypoint_task.mission_exec_times;
  ...
  wpInitData.RCLostAction =
    (unsigned char)request.waypoint_task.action_on_rc_lost;
  wpInitData.gimbalPitch =
    (unsigned char)request.waypoint_task.gimbal_pitch_mode;
  ...
  dji_sdk::MissionWpGetInfo::Response& response)
{
  ...
  for (int i = 0; i < info.indexNumber; i++)
  {
    ...
    response.waypoint_task.mission_waypoint[i].latitude =
      wpData.latitude * 180.0 / C_PI;
    response.waypoint_task.mission_waypoint[i].longitude =
      wpData.longitude * 180.0 / C_PI;
    response.waypoint_task.mission_waypoint[i].altitude = wpData.altitude;
    response.waypoint_task.mission_waypoint[i].damping_distance =
      wpData.damping;
    response.waypoint_task.mission_waypoint[i].target_yaw = wpData.yaw;
    response.waypoint_task.mission_waypoint[i].target_gimbal_pitch =
      wpData.gimbalPitch;
    ...
    response.waypoint_task.mission_waypoint[i].action_time_limit =
      wpData.actionTimeLimit;
    response.waypoint_task.mission_waypoint[i].waypoint_action.action_repeat =
      wpData.actionNumber + (wpData.actionRepeat << 4);

    std::copy(std::begin(wpData.commandList), std::end(wpData.commandList),
              response.waypoint_task.mission_waypoint[i]
                .waypoint_action.command_list.begin());

    std::copy(std::begin(wpData.commandParameter),
              std::end(wpData.commandParameter),
              response.waypoint_task.mission_waypoint[i]
                .waypoint_action.command_parameter.begin());
  }
  return true;
}
