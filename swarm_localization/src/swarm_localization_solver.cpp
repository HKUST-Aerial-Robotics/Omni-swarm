...
void SwarmLocalizationSolver::cutting_edges() {
    ...
    ROS_INFO("Edge Optimized DIS %d(%d) DET %d(%d) LOOPS", distance_count, total_distance_count, detection_count, total_detection_count, good_loops.size());
    /*
...
void SwarmLocalizationSolver::estimate_observability() {
    ...
    ROS_INFO("GOOD LOOPS NUM %d", good_loops);
...
/*
    ...
    ROS_INFO("loop DT%fms [TS%d]%d->[TS%d]%d; DTS a %4.3fms b %4.3fms LOOP:", (tsa - tsb).toSec()*1000, TSShort(tsa.toNSec()), _ida, TSShort(tsb.toNSec()), 
